...
def init_project_ctx(
    ...
    return ProjectContext(
        path, config_filepath, config_rel_path, config_format, vcs_type
    )
...
def _parse_cfg(cfg_buffer: typ.IO[str]) -> RawConfig:
    ...
    if hasattr(cfg_parser, "read_file"):
        ...
        cfg_parser.readfp(
            cfg_buffer
        )  # pylint: disable=deprecated-method ; python2 compat
...
def _compile_v2_file_patterns(raw_cfg: RawConfig) -> typ.Iterable[FilePatternsItem]:
    """Create internal/compiled representation of the file_patterns config field.
    ...
    for filepath, raw_patterns in _iter_glob_expanded_file_patterns(
        ...
        for raw_pattern in raw_patterns:
            if raw_pattern.startswith("["):
                errmsg = f"Invalid pattern {raw_pattern} for {filepath}. Character not valid in this position '[' "
...
def _parse_current_version_default_pattern(
    raw_cfg: RawConfig, raw_cfg_text: str
) -> str:
...
def _parse_raw_config(ctx: ProjectContext) -> RawConfig:
    ...
    if ctx.config_rel_path not in raw_cfg["file_patterns"]:
        ...
        raw_version_pattern = _parse_current_version_default_pattern(
            raw_cfg, raw_cfg_text
        )
...
def default_config(ctx: ProjectContext) -> str:
    ...
    if fmt == "cfg":
        ...
        raise ValueError(
            f"Invalid config_format='{fmt}', must be either 'toml' or 'cfg'."
        )
