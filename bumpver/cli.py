...
def _validate_date(date: typ.Optional[str], pin_date: bool) -> typ.Optional[dt.date]:
    ...
    try:
        ...
        logger.error(
            f"Invalid parameter --date='{date}', must match format YYYY-0M-0D.",
            exc_info=True,
        )
        sys.exit(1)
...
def _log_no_change(subcmd: str, version_pattern: str) -> None:
    is_semver = "{semver}" in version_pattern or (
        "MAJOR" in version_pattern
        and "MAJOR" in version_pattern
        and "PATCH" in version_pattern
    )
    if is_semver:
        logger.warning(
            f"bumpver {subcmd} [--major/--minor/--patch] required for use with SemVer."
        )
    else:
        available_flags = [
            f"--{part.lower()}"
            for part in ("MAJOR", "MINOR", "PATCH")
            if part in version_pattern
        ]
        if available_flags:
...
def _get_normalized_pattern(
    raw_pattern: str, version_pattern: typ.Optional[str]
) -> str:
    is_version_pattern_required = (
        "{version}" in raw_pattern or "{pep440_version}" in raw_pattern
    )
...
def version_options(function: typ.Callable) -> typ.Callable:
    decorators = [
        click.option(
            "--major", is_flag=True, default=False, help="Increment MAJOR component."
        ),
        click.option(
            "-m",
            "--minor",
            is_flag=True,
            default=False,
            help="Increment MINOR component.",
        ),
        click.option(
            "-p",
            "--patch",
            is_flag=True,
            default=False,
            help="Increment PATCH component.",
        ),
        click.option(
            "-t",
            "--tag",
        ...
        click.option(
            ...
            is_flag=True,
            default=False,
            help="Leave date components unchanged.",
        ),
        click.option(
            "--date",
            default=None,
...
def _grep_text(pattern: patterns.Pattern, text: str, color: bool) -> typ.Iterable[str]:
    ...
    for match in pattern.regexp.finditer(text):
        ...
        prefixed_lines = [
            f"{lines_offset + i:>4}: {line}" for i, line in enumerate(lines)
        ]
        yield "\n".join(prefixed_lines)
...
def show(verbose: int = 0, fetch: bool = True, env: bool = False) -> None:
    ...
    if env:
        version_info = v2version.parse_version_info(
            cfg.current_version, cfg.version_pattern
        )
...
def incr_dispatch(
    ...
    *,
    ...
    has_v1_part = any(f"{{{part}}}" in raw_pattern for part in v1_parts)
...
def _update(
    ...
    try:
        if cfg.is_new_pattern:
            new_v2_vinfo = v2version.parse_version_info(
                new_version, cfg.version_pattern
            )
            v2rewrite.rewrite_files(cfg.file_patterns, new_v2_vinfo)
        else:
            new_v1_vinfo = v1version.parse_version_info(
                new_version, cfg.version_pattern
            )
...
def init(verbose: int = 0, dry: bool = False) -> None:
    ...
    if dry:
        click.echo(
            f"Exiting because of '-d/--dry'. Would have written to {ctx.config_rel_path}:"
        )
...
def get_latest_vcs_version_tag(cfg: config.Config, fetch: bool) -> typ.Optional[str]:
    ...
    if cfg.is_new_pattern:
        version_tags = [
            tag for tag in all_tags if v2version.is_valid(tag, cfg.version_pattern)
        ]
    else:
        version_tags = [
            tag for tag in all_tags if v1version.is_valid(tag, cfg.version_pattern)
        ]
...
def update(
    ...
    if commit_message is None:
        ...
        commit_msg_template, _ = re.subn(
            r"\b(OLD|NEW)\b", r"{\1_VERSION}", commit_message
        )
