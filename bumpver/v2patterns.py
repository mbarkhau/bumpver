...
def _convert_to_pep440(version_pattern: str) -> str:
    ...
    pep440_pattern, _ = re.subn(r"[^a-zA-Z\d.\[\]]", "", pep440_pattern)
    ...
    for part_name in part_names:
        ...
        if is_numerical_part:
            ...
            is_zero_truncation_part = (
                part_index == 0 or pep440_pattern[part_index - 1] == "."
            )
...
def normalize_pattern(version_pattern: str, raw_pattern: str) -> str:
    ...
    if "{pep440_version}" in normalized_pattern:
        ...
        normalized_pattern = normalized_pattern.replace(
            "{pep440_version}", pep440_version_pattern
        )
...
def _replace_pattern_parts(pattern: str) -> str:
    ...
    for _, (start_idx, end_idx, named_part_pattern) in sorted(
        ...
        if end_idx <= last_start_idx:
            result_pattern = (
                result_pattern[:start_idx]
                + named_part_pattern
                + result_pattern[end_idx:]
            )
...
@utils.memo
def compile_pattern(
    version_pattern: str, raw_pattern: typ.Optional[str] = None
) -> Pattern:
...
def compile_patterns(
    version_pattern: str, raw_patterns: typ.List[str]
) -> typ.List[Pattern]:
    return [
        compile_pattern(version_pattern, raw_pattern) for raw_pattern in raw_patterns
    ]
