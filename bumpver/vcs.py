...
# TODO remove backport if python 3.7 is no longer supported
if sys.version_info >= (3, 8):
    from shlex import join as _shlex_join
else:

    def _shlex_join(split_command: typ.Iterable[str]) -> str:
        """Return a shell-escaped string from *split_command*."""
        return " ".join(shlex.quote(arg) for arg in split_command)


class VCSAPI:
    """Abstraction for git and mercurial."""
    ...
    def __call__(self, cmd_name: str, env: Env = None, **kwargs: str) -> str:
        ...
        cmd_parts = shlex.split(cmd_tmpl)
        cmd_parts = [p.format(**kwargs) for p in cmd_parts]
        cmd_str = _shlex_join(cmd_parts)
...
def assert_not_dirty(
    vcs_api: VCSAPI, filepaths: typ.Set[str], allow_dirty: bool
) -> None:
    dirty_files = vcs_api.status(required_files=filepaths)

    if dirty_files:
        logger.warning(
            f"{vcs_api.name} working directory is not clean. Uncommitted file(s):"
        )
    ...
    if dirty_pattern_files:
        logger.error("Not committing when pattern files are dirty:")
