...
def test_v1_rewrite_lines():
    ...
    lines = v1rewrite.rewrite_lines(
        patterns, new_vinfo, ['__version__ = "v201809.0002-beta"']
    )
...
def test_v2_rewrite_lines():
    ...
    patterns = [
        v2patterns.compile_pattern(version_pattern, '__version__ = "{version}"')
    ]
    lines = v2rewrite.rewrite_lines(
        patterns, new_vinfo, ['__version__ = "v201809.0002-alpha"   ']
    )
    assert lines == ['__version__ = "v201811.0123-beta"   ']
    ...
    patterns = [
        v2patterns.compile_pattern(
            version_pattern, '__version__ = "YYYY0M.BLD[PYTAGNUM]"'
        )
    ]
...
def test_iter_file_globs():
    ...
    assert file_paths == {
        ...
        "setup.py",
        "README.rst",
        os.path.sep.join(("src", "module_v1", "__init__.py")),
        os.path.sep.join(("src", "module_v2", "__init__.py")),
    }
...
def test_v1_error_bad_pattern():
    with util.Project(project="b") as project:
        ...
        try:
            ...
            raise AssertionError("expected rewrite.NoPatternMatch")
...
def test_v1_iter_rewritten():
    ...
    file_patterns = {"bumpver/__init__.py": [init_pattern]}
...
def test_v2_iter_rewritten():
    ...
    file_patterns = {
        "bumpver/__init__.py": [
            v2patterns.compile_pattern(
                version_pattern, '__version__ = "YYYY.BUILD[-TAG]"'
            ),
        ]
    }
...
def test_v1_diff():
    ...
    pattern = v1patterns.compile_pattern(version_pattern, raw_pattern)
    file_patterns = {"bumpver/__init__.py": [pattern]}
    ...
    assert lines[:2] == [
        f"--- bumpver{os.path.sep}__init__.py",
        f"+++ bumpver{os.path.sep}__init__.py",
    ]
...
def test_remove_regex_chars():
    ...
    lines = v2rewrite.rewrite_lines(
        patterns, new_vinfo, ['__version__ = "2018.0002-alpha"   ']
    )
