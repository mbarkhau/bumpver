...
_xfail_no_git = pytest.mark.xfail(
    shutil.which("git") is None, reason="git is not available"
)
_ci_on_windows = platform.system() == "Windows" and os.getenv("CI") == "true"
_xfail_no_hg = pytest.mark.xfail(
    shutil.which("hg") is None or _ci_on_windows, reason="hg is not available"
)
...
def test_incr_semver(runner):
    ...
    for semver_pattern in semver_patterns:
        ...
        result = runner.invoke(
            cli.cli, ["test", "-vv", "--patch", old_version, semver_pattern]
        )
        assert result.exit_code == 0
        ...
        result = runner.invoke(
            cli.cli, ["test", "-vv", "--minor", old_version, semver_pattern]
        )
        assert result.exit_code == 0
        assert f"Version: {new_version}\n" in result.output
        ...
        result = runner.invoke(
            cli.cli, ["test", "-vv", "--major", old_version, semver_pattern]
        )
        assert result.exit_code == 0
        assert f"Version: {new_version}\n" in result.output
...
def test_incr_to_beta(runner):
    ...
    result = runner.invoke(
        cli.cli, ["test", "-vv", old_version, pattern, "--tag", "beta"]
    )
    assert result.exit_code == 0
    assert f"Version: {new_version}\n" in result.output
...
def test_incr_to_final(runner, caplog):
    ...
    result = runner.invoke(
        cli.cli, ["test", "-vv", old_version, pattern, "--tag", "final"]
    )
...
def test_incr_invalid(runner):
    ...
    result = runner.invoke(
        cli.cli, ["test", "-vv", old_version, pattern, "--tag", "alfa"]
    )
    assert result.exit_code == 1
...
def _update_config_val(filename, **kwargs):
    ...
    for key, val in kwargs.items():
        ...
        if replacement not in new_cfg_text:
            pattern = rf"^{key} = .*$"
            new_cfg_text = re.sub(
                pattern, replacement, new_cfg_text, flags=re.MULTILINE
            )
...
DEFAULT_VERSION_PATTERNS = [
    (
        '"vYYYY0M.BUILD[-TAG]"',
        _today.strftime("v%Y%m.1001-alpha"),
        _today.strftime("%Y%m.1001a0"),
    ),
    (
        '"vYYYY.BUILD[-TAG]"',
        _today.strftime("v%Y.1001-alpha"),
        _today.strftime("%Y.1001a0"),
    ),
    (
        '"{pycalver}"',
        _today.strftime("v%Y%m.1001-alpha"),
...
def test_update_semver_warning(runner, caplog, version_pattern):
    ...
    assert any(
        "[--major/--minor/--patch] required" in r.message for r in caplog.records
    )
...
def test_update_set_version(runner, caplog):
    ...
    log_msg = caplog.records[0].message
    assert (
        "Invalid version '2020.1234-invalid' for pattern 'vYYYY.BUILD[-TAG]'" in log_msg
    )
...
def test_get_diff(runner, version_pattern, cur_version, cur_pep440):
    ...
    if len(cur_pep440) == 11:
        ...
        raise AssertionError(len(cur_pep440))
...
def test_hg_commit_message(runner, caplog):
    ...
    expected = (
        "bump from v2019.1001-alpha (2019.1001a0) to v2019.1002-beta (2019.1002b0)"
    )
...
@pytest.mark.parametrize(
    "config_text",
    [
        SETUP_CFG_MULTIMATCH_FILE_PATTERNS_FIXTURE_V1,
...
@pytest.mark.parametrize(
    "version_pattern, old_version, expected, kwargs", ROLLOVER_TEST_CASES
)
